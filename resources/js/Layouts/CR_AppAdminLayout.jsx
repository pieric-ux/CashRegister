    ...
    const RespNavLink = (
        <>
            ...
            <ResponsiveNavLink href={route('workstations.index', application.slug)} active={route().current('workstations.index', application.slug)}>
                {GlobalTranslations.workstations}
            </ResponsiveNavLink>
            <ResponsiveNavLink href={route('categories.index', application.slug)} active={route().current('categories.index', application.slug)}>
                {GlobalTranslations.categories}
            </ResponsiveNavLink>
            <ResponsiveNavLink method="post" href={route('logout')} as="button">
                {GlobalTranslations.logout}
            </ResponsiveNavLink>
        </>
    );
    const DropdownLink = (
        <>
            <Dropdown.Link href={route('dashboard')}>{GlobalTranslations.home}</Dropdown.Link>
            <Dropdown.Link href={route('profile.edit')}>{GlobalTranslations.profile}</Dropdown.Link>
            <Dropdown.Link href={route('logout')} method="post" as="button">{GlobalTranslations.logout}</Dropdown.Link>
        </>
    );
        ...
        <div className="bg-gray-100 dark:bg-gray-900 transition ease-linear duration-300">
            ...
            <div className="flex h-screen overflow-hidden">
                ...
                <div className="relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden">
                    {/* Header Start*/}
                    <Header user={auth.customer} title={application.name} avatarPath={auth.avatarPath} RespNavLink={RespNavLink} DropdownLink={DropdownLink} sideBarOpen={sideBarOpen} setSideBarOpen={setSideBarOpen} GlobalTranslations={GlobalTranslations} />
